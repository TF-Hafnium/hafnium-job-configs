...
from config import MESSAGE_TOKENS
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="hafnium-lts-triage", description="check patches for LTS candidacy")
    ...
    csv_columns = ["index", "commit id in the master branch", "committer date", "commit summary",
                   "score", "Gerrit Change-Id", "patch link for the LTS branch",
                   "patch link for the master branch", "To be cherry-picked"]
    csv_data = []
    ...
    for cmt in repo.iter_commits('master'):
        score = 0

        # if we find a same Change-Id among the ones we collected from the LTS branch
        # then we have seen all the new patches in the master branch, so we should exit.
        change_id_match = re.search(r'Change-Id:\s*(\w+)', cmt.message)
        if change_id_match:
        ...
        if score > 0:
            ...
            # Append data to CSV
            csv_data.append({
                "commit id in the master branch": cmt.hexsha,
                ...
                "patch link for the master branch": gerrit_links.get("master", "N/A"),
